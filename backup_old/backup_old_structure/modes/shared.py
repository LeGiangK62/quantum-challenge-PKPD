    ...
    def compute_loss(self, batch):
        ...
        if hasattr(self.model, 'forward_pk') and hasattr(self.model, 'forward_pd'):
            if is_pd_data:
                ...
                from utils.helpers import get_pk_input_dim
                pk_input_dim = get_pk_input_dim(self.config)
                ...
                # Adjust tensor dimensions using helper function
                pk_pred_orig, target_pk = self._ensure_tensor_compatibility(pk_pred_orig, target)
                pd_pred_orig, target_pd = self._ensure_tensor_compatibility(pd_pred_orig, target)
            ...
            if self.config.use_mixup and torch.rand(1).item() < self.config.mixup_prob:
                ...
                from utils.helpers import get_pk_input_dim
                pk_input_dim_mix = get_pk_input_dim(self.config)
                
                # Apply dual mixup with consistent lambda for PK and PD
                mixed_x, y_a_pk, y_b_pk, y_a_pd, y_b_pd, lam = self.apply_dual_mixup(
                    x, target_pk_mix, target_pd_mix, self.config.mixup_alpha
                )
                
                # Extract PK input for forward pass
                mixed_x_pk = mixed_x[:, :pk_input_dim_mix]
                mixed_x_pd = mixed_x
                ...
                # Adjust tensor dimensions for mixup using helper function
                pk_pred_mix, y_a_pk = self._ensure_tensor_compatibility(pk_pred_mix, y_a_pk)
                pk_pred_mix, y_b_pk = self._ensure_tensor_compatibility(pk_pred_mix, y_b_pk)
                pd_pred_mix, y_a_pd = self._ensure_tensor_compatibility(pd_pred_mix, y_a_pd)
                pd_pred_mix, y_b_pd = self._ensure_tensor_compatibility(pd_pred_mix, y_b_pd)
                
                # Mixup loss calculation with consistent lambda
                ...
                pk_loss_mix = lam * torch.nn.functional.mse_loss(pk_pred_mix, y_a_pk_flat) + (1 - lam) * torch.nn.functional.mse_loss(pk_pred_mix, y_b_pk_flat)
                pd_loss_mix = lam * torch.nn.functional.mse_loss(pd_pred_mix, y_a_pd_flat) + (1 - lam) * torch.nn.functional.mse_loss(pd_pred_mix, y_b_pd_flat)
