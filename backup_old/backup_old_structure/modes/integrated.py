    ...
    def compute_loss(self, batch):
        ...
        if has_forward_pk and has_forward_pd:
            if is_pk_data:
                ...
                pd_outs_orig, z_pd_orig, _ = self.model.forward_pd(pd_batch, pk_outs_orig, z_pk_orig)
            ...
            if self.config.use_mixup and torch.rand(1).item() < self.config.mixup_prob:
                ...
                # Extract PK and PD inputs for forward pass
                mixed_x_pk = mixed_x[:, :pk_input_dim_mix]
                mixed_x_pd = mixed_x[:, :pd_input_dim_mix]
